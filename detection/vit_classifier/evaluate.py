#!/usr/bin/env python3
"""
Evaluate the supervised ViT classifier per generator.

Anomaly score: sigmoid(logit) — probability the model assigns to class 1 (fake).
Protocol mirrors the VAE evaluators:
...
def collect_scores(model, loader, device):
    """Return (fake_probs, labels) — sigmoid output as anomaly score."""
    model.eval()
...
def evaluate_generator(model, data_dir, generator, device, batch_size, num_workers):
    ...
    thresh_ds = ClassifierEvalDataset(
        data_dir, generator, subset="thresh", seed=seed, transform=eval_transform
    )
    test_ds = ClassifierEvalDataset(
        data_dir, generator, subset="test", seed=seed, transform=eval_transform
    )
...
def main():
    ...
    parser.add_argument("--ckpt",        default=str(SCRIPT_DIR / "../../checkpoints/vit_classifier/best.pth"))
    parser.add_argument("--results_dir", default=str(SCRIPT_DIR / "../../checkpoints/vit_classifier"))
