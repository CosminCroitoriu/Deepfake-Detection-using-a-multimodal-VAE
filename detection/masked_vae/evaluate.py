#!/usr/bin/env python3
"""
Evaluate the Masked Concatenation VAE (Step 5).

Mirrors detection.concat_vae.evaluate — single forward pass per batch,
per-channel AUC for the 5-channel reconstruction, plus the combined-mean
anomaly score.

At evaluation time, masking is disabled (model.eval() turns it off via the
`self.training` check inside the model's forward). The model sees the full
input but has learned during training to handle modality dropout.
"""
import argparse
import json
...
from .model import MaskedConcatVAE
...
def collect_scores(model, loader, device):
    """Return (errors, errors_per_channel, labels) — single forward pass per batch."""
    model.eval()  # disables masking inside forward()
...
def main():
    ...
    parser.add_argument("--ckpt", default=str(SCRIPT_DIR / "../../checkpoints/masked_vae/best.pth"))
    parser.add_argument("--results_dir", default=str(SCRIPT_DIR / "../../checkpoints/masked_vae"))
    ...
    model = MaskedConcatVAE(latent_dim=args.latent_dim, mask_prob=args.mask_prob).to(device)
