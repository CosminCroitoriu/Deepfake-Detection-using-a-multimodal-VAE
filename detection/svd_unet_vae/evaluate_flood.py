#!/usr/bin/env python3
"""
Evaluate the flood-only SVD U-Net VAE against each flood generator separately.

Same protocol as detection.svd_unet_vae.evaluate: per-sample reconstruction
error (MSE) as the anomaly score, threshold chosen via Youden's Index on 20%
of fakes + real test split, metrics reported on the remaining 80%.

Loads the flood checkpoint (checkpoints/svd_unet_vae_flood/best.pth) and
defaults to the flood generators (sd_lora_flood, sd3_lora_flood).

Output: checkpoints/svd_unet_vae_flood/results.json

Usage:
  python -m detection.svd_unet_vae.evaluate_flood
  python -m detection.svd_unet_vae.evaluate_flood --generators sd_lora_flood
"""
import argparse
import json
...
from .dataset_flood import EvalDataset, GENERATOR_DIRS
...
def main():
    ...
    parser.add_argument("--ckpt", default=str(SCRIPT_DIR / "../../checkpoints/svd_unet_vae_flood/best.pth"))
    parser.add_argument("--results_dir", default=str(SCRIPT_DIR / "../../checkpoints/svd_unet_vae_flood"))
    parser.add_argument("--latent_dim", type=int, default=256)
    parser.add_argument("--batch", type=int, default=32)
    parser.add_argument("--num_workers", type=int, default=4)
    parser.add_argument("--generators", nargs="+", default=None,
                        help="Which generators to evaluate (default: auto-detect flood generators in data/fake/)")
    ...
    # auto-detect available flood generator directories
