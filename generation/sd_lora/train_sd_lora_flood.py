#!/usr/bin/env python3
"""
Flood-only fine-tune of Stable Diffusion v1.5 with LoRA (single-disaster ablation).

Identical to train_sd_lora.py except the dataset is restricted to the flood
...
spanning five disaster types plus generic damage/non-disaster photos. That
diversity exceeds the adapter's capacity and produces generic samples. Here the
same adapter covers one coherent mode (flood, ~3.2k images), which should yield
more faithful fakes. 3.2k is generous for a domain LoRA, so no augmentation is
applied and epochs are kept modest to avoid overfitting.

Writes to a separate checkpoint directory so the all-classes run is untouched.
...
Usage:
  python train_sd_lora_flood.py
  python train_sd_lora_flood.py --epochs 8 --batch_size 32
"""
import argparse
import json
...
def main():
    ...
    parser.add_argument("--output_dir", default=str(SCRIPT_DIR / "../../checkpoints/sd_lora_flood"))
